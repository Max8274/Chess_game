...
class ExampleUtility(Utility):
    ...
    def board_value(self, board: chess.Board):
        n_white = 0
        n_white += len(board.pieces(piece_type=chess.PAWN, color=chess.WHITE)) #pion
        n_white += len(board.pieces(piece_type=chess.BISHOP, color=chess.WHITE)) #loper
        n_white += len(board.pieces(piece_type=chess.KNIGHT, color=chess.WHITE)) #paard
        n_white += len(board.pieces(piece_type=chess.ROOK, color=chess.WHITE)) #toren
